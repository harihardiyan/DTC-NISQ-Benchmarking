# Mark 7: Pure Physics DTC Lifetime vs Noise (EPSILON=0.02, NOISE_P2=2*NOISE_P1)
# Pure Science & Anti-Cheat: no penalty terms, no magic constants; failures are logged.
...
CFG = {
    ...
    "TAU_MIN_GREEN": 100.0,
    "PLOT": True,
    "CSV_LEDGER": None  # e.g., "mark7_tau_ledger.csv" to enable CSV
}
...
# =========================
# Lifetime estimators (no penalty/fudge)
# =========================
...
def write_csv_header(path):
    ...
    with open(path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["NOISE_P1", "NOISE_P2", "EPSILON", "tau_fit", "model", "beta", "C", "fit_mse", "final_purity"])

def append_csv_row(path, p1, p2, epsilon, tau_fit, model, beta, C, mse, final_purity):
    import csv
    with open(path, "a", newline="") as f:
        w = csv.writer(f)
        w.writerow([f"{p1:.8f}", f"{p2:.8f}", f"{epsilon:.4f}", f"{tau_fit:.6f}", model, f"{beta:.6f}", f"{C:.6f}", f"{mse:.6e}", f"{final_purity:.6f}"])
...
def main():
    ...
    for p1 in p1_values:
        ...
        if cfg["CSV_LEDGER"]:
            append_csv_row(cfg["CSV_LEDGER"], p1, p2, cfg["EPSILON"], tau_fit, model_info[0], model_info[1], model_info[2], mse, float(mean_pur[-1]))
    ...
    if cfg["PLOT"]:
        ...
        plt.title("Mark 7: DTC lifetime vs noise (EPSILON=0.02, NOISE_P2=2*NOISE_P1)")
